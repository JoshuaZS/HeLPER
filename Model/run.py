...
class Runner(object):
    def __init__(self, params):
        ...
        self.optimizer = torch.optim.Adam(
            self.model.parameters(), lr=self.p.lr)
    ...
    def fit(self):
        ...
        scheduler = ReduceLROnPlateau(self.optimizer, mode='max', factor=0.1, patience=25, threshold=1e-4,
                                      threshold_mode='abs', min_lr=1e-4, verbose=True)
        for p in self.model.parameters():
            if p.requires_grad:
                print(p.size())
        self.logger.info("Params: %d", sum(p.numel() for p in self.model.parameters() if p.requires_grad))
        ...
        for epoch in range(self.p.max_epochs):
            ...
            scheduler.step(self.best_val_mrr)
            self.logger.info(
    ...
    def train(self):
        ...
        for step, (triplets, labels) in enumerate(train_iter):  # 共149689训练边
            if self.p.gpu >= 0:
    ...
    def predict(self, split='valid', mode='tail'):
        ...
        with torch.no_grad():
            ...
            for step, (triplets, labels) in enumerate(test_iter):
                triplets, labels = triplets.to("cuda"), labels.to("cuda")  # [B, 3]  [B, N]
    ...
    def get_model(self):
        model_name = self.p.name.split('_')[0].lower()
        self.p.drop1, self.p.drop2, self.p.drop3 = 0.2, 0.2, 0.3
        if model_name == 'conve':
            if (self.p.dataset == 'FB15k-237'):
                self.p.lr = 1e-4
                self.p.drop1, self.p.drop2, self.p.drop3 = 0.3, 0.2, 0.3
            ...
            if (self.p.dataset == 'FB15k-237'):
                ...
                self.p.lr = 5e-3
            model = HypER(self.num_ent, self.num_rels, params=self.p)
        elif model_name == 'tucker':
            if (self.p.dataset == 'FB15k-237'):
                self.p.lr = 5e-4
                self.p.rdim = 200
                self.p.drop1, self.p.drop2, self.p.drop3 = 0.3, 0.4, 0.5
            elif (self.p.dataset == 'wn18rr'):
                self.p.lr = 3e-3
                self.p.rdim = 30
            model = TuckER(self.num_ent, self.num_rels, params=self.p)
        elif model_name == 'lowfer':
            if (self.p.dataset == 'FB15k-237'):
                self.p.lr = 5e-4
                self.p.k = 100
                self.p.rdim = 200
                ...
                self.p.k = 30
                self.p.rdim = 30
            model = LowFER(self.num_ent, self.num_rels, params=self.p)
        elif model_name == 'helper':
            if (self.p.dataset == 'FB15k-237'):
                self.p.lr = 1e-3
                self.p.batch_size = 256
                self.p.k = 50
                self.p.in_ch, self.p.out_ch = 20, 20
                self.p.drop1, self.p.drop2, self.p.drop3 = 0.3, 0.4, 0.5
            elif (self.p.dataset == 'wn18rr'):
                self.p.lr = 3e-3
                self.p.k = 30
                self.p.in_ch, self.p.out_ch = 12, 24
            elif (self.p.dataset == 'yago'):
                self.p.lr = 3e-3
                self.p.k = 50
                self.p.in_ch, self.p.out_ch = 4, 6
            model = HeLpER(self.num_ent, self.num_rels, params=self.p)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', dest='max_epochs',
                        type=int, default=600, help='Number of epochs')
